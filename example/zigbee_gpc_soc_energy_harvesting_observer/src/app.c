...
#include "em_device.h"  // For NVIC_SystemReset()
#include "app.h"
...
#define LED_BLINK_INTERVAL_NO_NETWORK_MS    1000
#define LED_BLINK_INTERVAL_BTN_HOLD_MS      200
#define BTN_HOLD_TIMEOUT_MS                 2000

#define POWER_CONFIG_OFFSET_CMD_ID          0
#define POWER_CONFIG_OFFSET_DATA_TYPE       2
#define POWER_CONFIG_OFFSET_VALUE           3
#define BATTERY_VOLTAGE_ATTR_PAYLOAD_LEN    4

enum {
  COMMISSION_LED_STATE_OPERATING = 0,
  COMMISSION_LED_STATE_LEAVING
};
...
#endif // SL_CATALOG_ZIGBEE_MULTIRAIL_DEMO_PRESENT
static sl_zigbee_af_event_t nwk_management_event;
static sl_sleeptimer_timer_handle_t button_release_timer_handle;

static bool commissioning = false;
static bool btn1_long_pressed = false;
static uint8_t commission_led_state = COMMISSION_LED_STATE_OPERATING;
...
{
  uint32_t delay_ms =
    (commission_led_state
     == COMMISSION_LED_STATE_OPERATING) ? LED_BLINK_INTERVAL_NO_NETWORK_MS
    :LED_BLINK_INTERVAL_BTN_HOLD_MS;
...
extern bool
sli_zigbee_af_green_power_server_gp_sink_commissioning_mode_command_handler(
  uint8_t options,
...
static void sink_commissioning_mode_event_handler(sl_zigbee_af_event_t *event)
{
  if (sl_zigbee_af_network_state() == SL_ZIGBEE_JOINED_NETWORK) {
    uint8_t options =
      SL_ZIGBEE_AF_GP_SINK_COMMISSIONING_MODE_OPTIONS_INVOLVE_PROXIES \
      | ((!commissioning)
         ?SL_ZIGBEE_AF_GP_SINK_COMMISSIONING_MODE_OPTIONS_ACTION : 0);

    sli_zigbee_af_green_power_server_gp_sink_commissioning_mode_command_handler(
      options,
      // options - (Involve Proxy | Enter)
      0xFFFF,
      // addr
      0xFFFF,
      // addr
...
static void nwk_management_event_handler(sl_zigbee_af_event_t *event)
{
  if (sl_zigbee_af_network_state() == SL_ZIGBEE_JOINED_NETWORK) {
    if (btn1_long_pressed) {
...
{
  sl_zigbee_af_event_init(&commissioning_led_event,
                          commissioning_led_event_handler);
  sl_zigbee_af_isr_event_init(&finding_and_binding_event,
                              finding_and_binding_event_handler);
  sl_zigbee_af_isr_event_init(&sink_commissioning_mode_event,
                              sink_commissioning_mode_event_handler);
  sl_zigbee_af_isr_event_init(&nwk_management_event,
                              nwk_management_event_handler);
#if defined(SL_CATALOG_ZIGBEE_MULTIRAIL_DEMO_PRESENT)
  sl_zigbee_af_isr_event_init(&gp_transmit_complete_event,
...
void sl_zigbee_af_stack_status_cb(sl_status_t status)
{
  ...
  if (status == SL_STATUS_NETWORK_DOWN) {
    // Restart if the network is down
    NVIC_SystemReset();
  } else if (status == SL_STATUS_NETWORK_UP) {
    // Stop blinking led0 once network is up
    sl_zigbee_af_event_set_inactive(&commissioning_led_event);
    sl_zigbee_af_event_set_active(&finding_and_binding_event);
    commission_led_state = COMMISSION_LED_STATE_OPERATING;
  }

  led_turn_off(COMMISSIONING_STATUS_LED);
  // Disable commissioning if it is currently enabled.
  if (commissioning) {
...
void sl_zigbee_af_network_steering_complete_cb(sl_status_t status,
                                               ...
                                               uint8_t finalState)
{
  sl_zigbee_app_debug_println("%s network %s: 0x%X", "Join", "complete",
                              status);
 ...
 */
void sl_zigbee_af_green_power_server_commissioning_timeout_cb(
  uint8_t commissioningTimeoutType,
 ...
 * @param endpoints list of sink endpoints Ver.: always
 */
void sl_zigbee_af_green_power_server_pairing_complete_cb(
  uint8_t numberOfEndpoints,
  uint8_t *endpoints)
{
...
void sl_zigbee_af_green_power_server_pre_sink_pairing_cb(
  sli_zigbee_gp_comm_data_saved_t *commissioningGpd)
{
  if (commissioningGpd->preSinkCbSource
  ...
  *count = 0;
  ...
  for (uint8_t i = 0;
       i < totalEndpointCount
       && grouplist
       < (commissioningGpd->groupList
                                               ...
                                               RAIL_Events_t events)
{
  ...
  && (SL_ZIGBEE_AF_PLUGIN_IDLE_SLEEP_USE_BUTTONS == 0)
...
{
  sl_button_state_t btnState = SL_SIMPLE_BUTTON_DISABLED;
  if (handle == NULL) {
    return;
  }
  btnState = sl_button_get_state(handle);

  if (btnState == SL_SIMPLE_BUTTON_PRESSED) {
    if ((handle == SL_SIMPLE_BUTTON_INSTANCE(BUTTON1))
  ...
  } else if (btnState == SL_SIMPLE_BUTTON_RELEASED) {
    if (handle == SL_SIMPLE_BUTTON_INSTANCE(BUTTON0)) {
      ...
      sl_zigbee_af_event_set_active(&nwk_management_event);
    }
  }
}

#endif \
  // SL_CATALOG_SIMPLE_BUTTON_PRESENT && SL_ZIGBEE_AF_PLUGIN_IDLE_SLEEP_USE_BUTTONS == 0
...
void gpAppGpTxQueueInit(sl_cli_command_arg_t *arguments)
{
  ...
  RAIL_Handle_t h = sl_zigbee_af_multirail_demo_init(NULL,
                                                     ...
                                                     0xFFFF,
                                                     NULL);
  sl_zigbee_app_debug_println("Additional RAIL handle %sinitialized",
                              h ? "" : "not ");
}
...
void gpAppGpTxQueueSet(sl_cli_command_arg_t *arguments)
{
  ...
  uint8_t gpdAsdu[255];
  gpdAsduLength = sl_zigbee_copy_hex_arg(arguments,
                                         3,
                                         gpdAsdu,
                                         sizeof(gpdAsdu),
                                         false);
...
void gpAppGpGetTxQueueSize(sl_cli_command_arg_t *arguments)
{
  ...
  sl_zigbee_app_debug_println("Max Tx Queue Size = %d",
...
#endif
#define macToAppDelay(macTimeStamp) ((RAIL_GetTime() & 0x00FFFFFF) \
                                     - (macTimeStamp))
...
static sl_zigbee_gp_tx_queue_entry_t * get_gp_stub_tx_queue(
  ...
  uint8_t *asdu)
{
  ...
  if (sl_zigbee_af_multirail_demo_get_handle()
    ...
    sli_buffer_manager_buffer_t header = sli_zigbee_gpdf_make_header(true,
                                                                     NULL,
                                                                     &(
                                                                       sli_zigbee_gp_tx_queue
                                                                       .addr));
    ...
    sl_legacy_buffer_manager_set_linked_buffers_length(header,
                                                       sl_legacy_buffer_manager_message_buffer_length(
                                                         header)
                                                       + dataLength);
    // Add the payload
    ...
    if (macToAppDelay(macTimeStamp) < GP_RX_OFFSET_USEC) {
      ...
      if (((packetData[7] & 0xC3) == 0x80)   // FC : ExtFC(b7) = 1 | AC(b6) = 0 | Frame type(b1-b0) = 0 = Data frame
        ...
        (void) memcpy(&gpdAddr.id.sourceId, &packetData[9],
                      sizeof(sl_zigbee_gp_source_id_t));
      }
      // Is there a queued response for this source ID?
      uint8_t outPktLength;
      uint8_t outPkt[128];
      sl_zigbee_gp_tx_queue_entry_t *entry = get_gp_stub_tx_queue(&gpdAddr,
                                                                  &outPktLength,
                                                                  (uint8_t *)&outPkt);
      if (entry) {
                                       ...
                                       int16u bufLen)
{
  if ((clusterId == ZCL_POWER_CONFIG_CLUSTER_ID)
      && (bufLen == BATTERY_VOLTAGE_ATTR_PAYLOAD_LEN)) {
    uint16_t *cmd_id = (uint16_t *)&buffer[POWER_CONFIG_OFFSET_CMD_ID];
    uint8_t *data_type = &buffer[POWER_CONFIG_OFFSET_DATA_TYPE];
    if ((*cmd_id == ZCL_BATTERY_VOLTAGE_ATTRIBUTE_ID)
        && (*data_type == ZCL_INT8U_ATTRIBUTE_TYPE)) {
      sl_zigbee_app_debug_println(
        "*** Battery Voltage of remote node: %.1f V ***",
        (float)buffer[POWER_CONFIG_OFFSET_VALUE]
        / 10);
    }
  }
  return false;
...
{
  if (btn1_long_pressed) {
    if (!sl_zigbee_af_event_is_scheduled(&commissioning_led_event)) {
      sl_zigbee_af_event_set_active(&commissioning_led_event);
    }
  }
}
                                 ...
                                 void *contextData)
{
  ...
  btn1_long_pressed = true;
  commission_led_state = COMMISSION_LED_STATE_LEAVING;
}
