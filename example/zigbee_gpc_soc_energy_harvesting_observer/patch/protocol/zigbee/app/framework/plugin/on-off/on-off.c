...
// For the de-bouncing feature
#define DEBOUNCE_INTERVAL_MS        1000
#if (DEBOUNCE_INTERVAL_MS > 0)
                                                               ...
                                                               initiatedByLevelChange)
{
  ...
  sl_zigbee_af_on_off_cluster_println("On/Off set value: %02X %02X",
                                      endpoint,
                                      command);
...
#if (DEBOUNCE_INTERVAL_MS > 0)
  // If the current command is TOGGLE, go to debouncing check process
  if (command == ZCL_TOGGLE_COMMAND_ID) {
    bool timer_is_running;
    sl_sleeptimer_is_timer_running(&debounce_timer,
                                   &timer_is_running);

    if (timer_is_running == false) {
      // Start a timer for debouncing
      // We don't need to implement the callback for debouncing, so the 3rd parameter can be NULL.
      sl_sleeptimer_start_timer_ms(&debounce_timer,
      ...
      // If debounce_timer is operating, exit the function so the attribute is not changed
      sl_zigbee_af_on_off_cluster_println("*** De-bouncing initiated ***");
      return SL_ZIGBEE_ZCL_STATUS_SUCCESS;
    }
  }
#endif
  ...
  sl_zigbee_af_on_off_cluster_println("Toggle on/off from %02X to %02X",
                                      currentValue,
                                      newValue);
  ...
  if (newValue) {
    ...
    if (!initiatedByLevelChange
        && sl_zigbee_af_contains_server(endpoint,
...
{
  sl_zigbee_af_status_t status =
    sl_zigbee_af_on_off_cluster_set_value_cb(sl_zigbee_af_current_endpoint(),
                                             ZCL_OFF_COMMAND_ID,
                                             false);
#ifdef SL_CATALOG_ZIGBEE_ZLL_ON_OFF_SERVER_PRESENT
  if (status == SL_ZIGBEE_ZCL_STATUS_SUCCESS) {
...
uint32_t sl_zigbee_af_on_off_cluster_server_command_parse(
