...
// Minor change in this file in order to complete GP channel negotiation process
#define KINETIC_SWITCH_GP_CHANNEL_REQUEST
                                            ...
                                            uint8_t *cmd)
{
  ...
  // Options field ------------------------------------------------------------
  cmd[index] =
    (SL_ZIGBEE_AF_PLUGIN_APPS_MAC_SEQ
     ? SL_ZIGBEE_GPD_COM_OPT_MAC_SEQ_NBR_CAP_MASK : 0)                                                 // b0 - MAC Seq Nbr capability sub-field
    | (SL_ZIGBEE_AF_PLUGIN_APPS_BIDIR_CAPABLE
       ? SL_ZIGBEE_GPD_COM_OPT_RX_CAP_MASK : 0)                                                        // b1 - RxOn capability sub-field
    | (SL_ZIGBEE_AF_PLUGIN_APPS_APP_INFO
       ? SL_ZIGBEE_GPD_COM_OPT_APP_INFO_MASK : 0)                                                      // b2 - Application Information sub-field
    | (SL_ZIGBEE_AF_PLUGIN_APPS_PAN_ID_REQUEST
       ? SL_ZIGBEE_GPD_COM_OPT_PANID_REQ_MASK : 0);                                                    // b4 - Pan ID Request sub-field
  ...
  if ((keyType == SL_ZIGBEE_GPD_SECURITY_KEY_TYPE_NWK)
      ...
      || (SL_ZIGBEE_AF_PLUGIN_APPS_SECURITY_LEVEL
          == SL_ZIGBEE_GPD_SECURITY_LEVEL_NONE)) {
    // No setting of Key Request flag
  } else {
    // else set the key request based on the application configuration
    cmd[index] |=
      (SL_ZIGBEE_AF_PLUGIN_APPS_KEY_REQUEST
       ? SL_ZIGBEE_GPD_COM_OPT_KEY_REQ_MASK : 0);                                                      // b5 - GP Security Key Request sub-field
  }
  cmd[index] |=
    (SL_ZIGBEE_AF_PLUGIN_APPS_FIXED_LOCATION
     ? SL_ZIGBEE_GPD_COM_OPT_FIXED_LOC_MASK : 0);                                                      // b6 - Fixed location sub-field

  // Extended option sub-field
#if defined(SL_ZIGBEE_AF_PLUGIN_APPS_SECURITY_LEVEL) \
  && (SL_ZIGBEE_AF_PLUGIN_APPS_SECURITY_LEVEL        \
      == SL_ZIGBEE_GPD_SECURITY_LEVEL_NONE)
...
#elif defined(SL_ZIGBEE_AF_PLUGIN_APPS_SECURITY_LEVEL) \
  && (SL_ZIGBEE_AF_PLUGIN_APPS_SECURITY_LEVEL          \
      > SL_ZIGBEE_GPD_SECURITY_LEVEL_NONE)
  ...
  cmd[index] = (SL_ZIGBEE_GPD_EXT_COM_OPT_SECURITY_LEVEL_CAP_MASK
                & SL_ZIGBEE_AF_PLUGIN_APPS_SECURITY_LEVEL)                                                   // b0-b1 Security Level  sub-field
               | (keyType << SL_ZIGBEE_GPD_EXT_COM_OPT_KEY_TYPE_OFFSET)                                  // b2-b3-b4 Key type  sub-field
               | (keyPresentInComRequest
                  ? SL_ZIGBEE_GPD_EXT_COM_OPT_KEY_PRESENT_MASK : 0)                                      // b5 Key Present  sub-field
               | (SL_ZIGBEE_AF_PLUGIN_APPS_KEY_ENCRYPT
                  ? SL_ZIGBEE_GPD_EXT_COM_OPT_KEY_ENCRYPT_MASK : 0)                                          // b6 Key encrypted sub-field
...
#if defined(SL_ZIGBEE_AF_PLUGIN_APPS_KEY_ENCRYPT) \
    && (SL_ZIGBEE_AF_PLUGIN_APPS_KEY_ENCRYPT == true)
...
#else
    (void) memcpy(&(cmd[index]),
                  gpd->securityKey,
                  SL_ZIGBEE_AF_PLUGIN_APPS_KEY_LENGTH);
#endif
  }
...
#if defined(SL_ZIGBEE_AF_PLUGIN_APPS_APP_INFO) \
  && (SL_ZIGBEE_AF_PLUGIN_APPS_APP_INFO == true)
  ...
  *appInfo |= SL_ZIGBEE_GPD_APP_INFO_COM_CMDLIST_MASK;                        // b2 GpdCmd List present
  ...
  || (SL_ZIGBEE_AF_PLUGIN_APPS_APPS_OUT_CLUS_LIST_LENGTH > 0)
...
static uint8_t applicationDescriptionState =
  SL_ZIGBEE_GPD_COMM_STATE_APP_DESC_STEP_SEND_COMMISSIONING_REQ;
...
static bool sendApplicationDescriptionCommand(sl_zigbee_gpd_t_t *gpd)
{
  ...
  uint8_t commandLength =
    gpdBuildCommissioningApplicationDescriptionCommand(command,
                                                       &
                                                       last);

#if defined SL_ZIGBEE_AF_PLUGIN_APPS_BIDIR_CAPABLE \
  && (SL_ZIGBEE_AF_PLUGIN_APPS_BIDIR_CAPABLE == true)
...
static void gpdCommissioningStateMachineWithAppDescription(
  sl_zigbee_gpd_t_t *gpd)
{
  ...
  // formation.
  if (applicationDescriptionState
      == SL_ZIGBEE_GPD_COMM_STATE_APP_DESC_STEP_SEND_COMMISSIONING_REQ) {
    gpd->rxAfterTx = false;
    for (int i = 0;
         i < SL_ZIGBEE_AF_PLUGIN_APPS_GPD_APP_NUMBER_OF_COMMISSIONING_REQUEST;
         i++) {
      sendCommissioningRequest(gpd);
    }
    applicationDescriptionState =
      SL_ZIGBEE_GPD_COMM_STATE_APP_DESC_STEP_SEND_APP_DESCRIPTION_REQ;
  } else if (applicationDescriptionState
             == SL_ZIGBEE_GPD_COMM_STATE_APP_DESC_STEP_SEND_APP_DESCRIPTION_REQ)
  {
    ...
    if (last) {
      ...
      applicationDescriptionState =
        SL_ZIGBEE_GPD_COMM_STATE_APP_DESC_STEP_SEND_APP_DESCRIPTION_FINAL_REQ;
    }
  } else if (applicationDescriptionState
             ==
             SL_ZIGBEE_GPD_COMM_STATE_APP_DESC_STEP_SEND_APP_DESCRIPTION_FINAL_REQ)
...
static uint8_t chReqSubstate =
  SL_ZIGBEE_GPD_COMM_STATE_CHANNEL_REQ_ON_ALL_CHANNELS;
...
static void gpdCommissioningChannelRequestStateOnAllChannel(
  sl_zigbee_gpd_t_t *gpd)
{
  // Sub state 1
  ...
  for (int j =
       ...
       j
       <
       SL_ZIGBEE_AF_PLUGIN_APPS_GPD_APP_NUMBER_OF_CHANNEL_REQUEST_PER_CHANNEL_WITH_AC_SET;
       j++) {
...
static void gpdCommissioningChannelRequestOnRxChannel(sl_zigbee_gpd_t_t *gpd)
{
  ...
  for (int i = 0;
       i
       < SL_ZIGBEE_AF_PLUGIN_APPS_GPD_APP_NUMBER_OF_CHANNEL_REQUEST_PER_CHANNEL;
       i++) {
...
#ifdef KINETIC_SWITCH_GP_CHANNEL_REQUEST
    // Utilize the flag SL_ZIGBEE_GPD_APP_STATE_CHANNEL_REQUEST to record current GPD state
    // This will be stored in NVM, so the GPD is able to resume next commissioning step
    gpd->gpdState = SL_ZIGBEE_GPD_APP_STATE_CHANNEL_REQUEST;
#endif
  } else if (chReqSubstate
             == SL_ZIGBEE_GPD_COMM_STATE_CHANNEL_REQ_ON_RX_CHANNEL) {
                                                 ...
                                                 uint8_t *pRxbuffer)
{
  ...
  bool panIdPresent = (options
                       & SL_ZIGBEE_GPD_COM_REP_OPT_PANID_PRESENT_MASK) ? true
                      : false;
  bool keyPresent = (options
                     & SL_ZIGBEE_GPD_COM_REP_OPT_KEY_PRESENT_MASK) ? true
                    : false;
  bool keyEncrypted = (options
                       & SL_ZIGBEE_GPD_COM_REP_OPT_KEY_ENCRYPT_MASK) ? true
                      : false;
  uint8_t securityLevel = (options
                           & SL_ZIGBEE_GPD_COM_REP_OPT_SECURITY_LEVEL_MASK) >>
                          SL_ZIGBEE_GPD_COM_REP_OPT_SECURITY_LEVEL_OFFSET;
  uint8_t securityKeyType = (options
                             & SL_ZIGBEE_GPD_COM_REP_OPT_KEY_TYPE_MASK) >>
                            SL_ZIGBEE_GPD_COM_REP_OPT_KEY_TYPE_OFFSET;
...
{
  if (SUCCESS
      == sl_zigbee_gpd_process_commissioning_reply(sl_zigbee_gpd_get_gpd(),
                                                   length,
                                                   commReplyPayload)) {
    sl_zigbee_gpd_set_state(
      SL_ZIGBEE_GPD_APP_STATE_COMMISSIONING_REPLY_RECEIVED);
  }
}
...
#ifdef KINETIC_SWITCH_GP_CHANNEL_REQUEST
  } else if (gpd->gpdState == SL_ZIGBEE_GPD_APP_STATE_CHANNEL_REQUEST) {
    // Send Channel Request on RX channel with rxAfterTx as true
    gpdCommissioningChannelRequestOnRxChannel(gpd);
    chReqSubstate = SL_ZIGBEE_GPD_COMM_STATE_CHANNEL_REQ_ON_ALL_CHANNELS;
#endif
  } else if (gpd->gpdState == SL_ZIGBEE_GPD_APP_STATE_CHANNEL_RECEIVED) {
    gpdBidirCommissioningStateMachine(gpd);
  } else if (gpd->gpdState
             == SL_ZIGBEE_GPD_APP_STATE_COMMISSIONING_REPLY_RECEIVED) {
    sendSuccess(gpd);
    sl_zigbee_gpd_set_state(
